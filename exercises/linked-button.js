try {
	...
	testComponent.attributesToTest = [ 'nodeName', 'innerText', 'target', 'childElementCount', 'href', 'children' ];

	const testCases = [
		{
			name: 'first',
			args: [ [ 'cats', 'https://thecatsite.com/' ] ],
			expected: {
				nodeName: 'A',
				childElementCount: 1,
				target: '_blank',
		...
		{
			name: 'third',
			args: [ [ 'badgers', 'https://www.badgerbadgerbadger.com/' ] ],
			expected: {
				nodeName: 'A',
				childElementCount: 1,
				target: '_blank',
	...
	testComponent(linkedButton, testCases, div);
